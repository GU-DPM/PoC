...
from DPM_plot import DPM_plot_df2pdf, DPM_plot_multi
...
def DPM_run_drugmis_processing(Num_drug=2, pathload='', Strategy_name=None, Simduration=1800, use_parallel=False):
    def DPM_run_drugmis_processing_1(i_stoptime_):
        ...
        i_dosage = DPM_read_dosage_csv(i_file_dosage, Strategy_name, None)
...
def DPM_run_drugmis_analysis(mis=None, Strategy_name=None, Simduration=SIMDURATION_DEFAULT_VAL, filename_pattern='result',
    ...
    def DPM_run_drugmis_analysis_LOD_1(pathload_, mis_flag=False):
    ...
    def DPM_run_drugmis_analysis_LOD_2(stoptime_, dosage_, hz_ratio, p, hz, firstuse, average_dose_intensity, average_move_num):
        ...
        hz_ratio.append(hz_ratio_), p.append(p_)
        for i_strategy in Strategy_name:
            hz[i_strategy].append(hz_[i_strategy])
        ...
        return hz_ratio, p, hz, firstuse, average_dose_intensity, average_move_num

    def DPM_run_drugmis_analysis_LOD_3(result_, f_, mis_=None):
        ...
        hz_ratio_ci_ = DPM_PeterMon_run_ci(hz_ratio_)
        ...
        for i_strategy in Strategy_name:
            ...
            hz_ci_[i_strategy] = DPM_PeterMon_run_ci(hz_[i_strategy])

        frate = None
        if f_ == 'fn':
            frate = sum([i_p > 0.025 for i_p in p_])/len(p_) * 100
        elif f_ == 'fp':
            frate = sum([i_p < 0.025 for i_p in p_])/len(p_) * 100
    ...
    def DPM_run_drugmis_analysis_LOD_4(result_Ben_, result_NoBen_, result_Ben_mis_, result_NoBen_mis_, colname_, filename_, color_, linestyle_,
                                       name_, format_sci=False, plot=True):
        data = list()
        if type(result_Ben_) is dict:
            val_BenDPM = [result_Ben_['strategy2.2']] + [i_val['strategy2.2'] for _, i_val in result_Ben_mis_.items()]
            ...
            val_BenCPM = [result_Ben_['strategy0']] + [i_val['strategy0'] for _, i_val in result_Ben_mis_.items()]
            ...
            val_NoBenDPM = [result_NoBen_['strategy2.2']] + [i_val['strategy2.2'] for _, i_val in result_NoBen_mis_.items()]
            ...
            val_NoBenCPM = [result_NoBen_['strategy0']] + [i_val['strategy0'] for _, i_val in result_NoBen_mis_.items()]
        ...
        col_names_ = [colname_]
        colname_ = ['control', '5x', '10x', '30x', 'div5', 'div10', 'div30']
        ...
        if plot:
            ...
            plt.savefig(filename_[:filename_.rfind('/')+1] + name_ + ' fig.pdf', format='pdf')
            plt.close()
        return
    ...
    if not file_exist:
        ...
        para, stoptime, dosage, setind, km = DPM_run_drugmis_analysis_LOD_1(pathload)
        ...
        for i_mis in mis:
            ...
            _, i_stoptime_mis, i_dosage_mis, _, i_km_mis = DPM_run_drugmis_analysis_LOD_1(i_pathload, True)
            '''Use only the 'total' set in the mis_km'''
            ...
            for i_set in setname:
                if i_set == 'total':
                    ...
                    for i_strategy in Strategy_name:
                        ...
                        if i_strategy == 'strategy0':
                            # choose the initially administered drug.
        ...
        with tqdm(total=nsample, ncols=150, desc='Runing...') as pbar:
            for i in range(nsample):
                ...
                hz_ratio_Ben, p_Ben, hz_Ben, firstuse_Ben, average_dose_intensity_Ben, average_move_num_Ben = \
                    DPM_run_drugmis_analysis_LOD_2(i_stoptime_Ben,
                                                   i_dosage_Ben,
                                                   hz_ratio_Ben,
                                                   p_Ben,
                                                   hz_Ben,
                                                   firstuse_Ben,
                                                   average_dose_intensity_Ben,
                                                   average_move_num_Ben)
                # NoBen
                hz_ratio_NoBen, p_NoBen, hz_NoBen, firstuse_NoBen, average_dose_intensity_NoBen, average_move_num_NoBen = \
                    DPM_run_drugmis_analysis_LOD_2(i_stoptime_NoBen,
                                                   i_dosage_NoBen,
                                                   hz_ratio_NoBen,
                                                   p_NoBen,
                                                   hz_NoBen,
                                                   firstuse_NoBen,
                                                   average_dose_intensity_NoBen,
                                                   average_move_num_NoBen)
                ...
                for i_mis in mis:
                    hz_ratio_Ben_mis[i_mis], p_Ben_mis[i_mis], hz_Ben_mis[i_mis], firstuse_Ben_mis[i_mis], \
                        ...
                        DPM_run_drugmis_analysis_LOD_2(i_stoptime_Ben_mis[i_mis],
                                                       i_dosage_Ben_mis[i_mis],
                                                       hz_ratio_Ben_mis[i_mis],
                                                       p_Ben_mis[i_mis],
                                                       hz_Ben_mis[i_mis],
                                                       firstuse_Ben_mis[i_mis],
                                                       average_dose_intensity_Ben_mis[i_mis],
                    ...
                    hz_ratio_NoBen_mis[i_mis], p_NoBen_mis[i_mis], hz_NoBen_mis[i_mis], firstuse_NoBen_mis[i_mis], \
                        ...
                        DPM_run_drugmis_analysis_LOD_2(i_stoptime_NoBen_mis[i_mis],
                                                       i_dosage_NoBen_mis[i_mis],
                                                       hz_ratio_NoBen_mis[i_mis],
                                                       p_NoBen_mis[i_mis],
                                                       hz_NoBen_mis[i_mis],
                                                       firstuse_NoBen_mis[i_mis],
                                                       average_dose_intensity_NoBen_mis[i_mis],
                                                       average_move_num_NoBen_mis[i_mis])
                pbar.update(1)
    ...
    hz_ratio_Ben, hz_ratio_ci_Ben, falseneg_Ben, hz_Ben, hz_ci_Ben, firstuse_Ben, average_dose_intensity_Ben, average_move_num_Ben = \
        DPM_run_drugmis_analysis_LOD_3(result_Ben, 'fn')
    hz_ratio_NoBen, hz_ratio_ci_NoBen, falsepos_NoBen, hz_NoBen, hz_ci_NoBen, firstuse_NoBen, average_dose_intensity_NoBen, \
        average_move_num_NoBen = DPM_run_drugmis_analysis_LOD_3(result_NoBen, 'fp')
    ...
    for i_mis in mis:
        hz_ratio_Ben_mis[i_mis], hz_ratio_ci_Ben_mis[i_mis], falseneg_Ben_mis[i_mis], hz_Ben_mis[i_mis], hz_ci_Ben_mis[i_mis], \
            ...
            = DPM_run_drugmis_analysis_LOD_3(result_Ben_mis, 'fn', mis_=i_mis)
        ...
        hz_ratio_NoBen_mis[i_mis], hz_ratio_ci_NoBen_mis[i_mis], falsepos_NoBen_mis[i_mis], hz_NoBen_mis[i_mis], hz_ci_NoBen_mis[i_mis], \
            ...
            = DPM_run_drugmis_analysis_LOD_3(result_NoBen_mis, 'fp', mis_=i_mis)
    ...
    for i_setname in setname:
        ...
        DPM_plot_KM(km_ref, km_treat, par, i_setname)
        ...
        for i_mis in mis:
            if km_mis[i_setname][i_mis]['strategy0']['median_survival'] == np.inf:
                km_mis[i_setname][i_mis]['strategy0']['median_survival'] = Simduration
            if km_mis[i_setname][i_mis]['strategy2.2']['median_survival'] == np.inf:
                km_mis[i_setname][i_mis]['strategy2.2']['median_survival'] = Simduration
            km_cpm[i_setname][i_mis] = km_mis[i_setname][i_mis]['strategy0']
            km_dpm[i_setname][i_mis] = km_mis[i_setname][i_mis]['strategy2.2']

        DPM_plot_KM_multi(km_cpm[i_setname], par_multi, i_setname+' cpm')
        i_pathsave = os.path.join(pathsave_km, i_setname+' cpm.pdf')
        plt.savefig(i_pathsave, format='pdf')
        plt.close()

        DPM_plot_KM_multi(km_dpm[i_setname], par_multi, i_setname+' dpm')
        i_pathsave = os.path.join(pathsave_km, i_setname+' dpm.pdf')
        plt.savefig(i_pathsave, format='pdf')
        plt.close()
    '''1 par'''
    '''At 30x during the simulation, CPM improves'''
    ratio = 30
    para = para_sel['./30x_atsim/']['strategy0']
    ...
    '''At div30 during the simulation, DPM worsens'''
    ratio = 1/30
    para = para_sel['./div30_atsim/']['strategy2.2']
    para['Num_drug'] = NUM_DRUG_DEFAULT_VAL
    mis_para = deepcopy(para)
    mis_para['Sa.S.D2.'] = mis_para['Sa.S.D2.'] * ratio
    mis_para['Sa.R1.D2.'] = mis_para['Sa.R1.D2.'] * ratio
    ...
    col_name = 'Drug2 introduction\nmove'
    DPM_run_drugmis_analysis_LOD_4(firstuse_Ben, firstuse_NoBen, firstuse_Ben_mis, firstuse_NoBen_mis, col_name, filename_drug2_intromove,
                                   color, linestyle, name)
    # Average drug2 intensity
    name = 'average drug 2 intensity'
    filename_drug2_intensity = os.path.join(pathsave, f'{name}.pdf')
    col_name = 'Drug2 intensity'
    DPM_run_drugmis_analysis_LOD_4(average_dose_intensity_Ben, average_dose_intensity_NoBen, average_dose_intensity_Ben_mis,
                                   average_dose_intensity_NoBen_mis, col_name, filename_drug2_intensity, color, linestyle, name)
    # Drug 2 intensity weighted average
    name = 'drug 2 intensity weighted average'
    filename_drug2_intensity_weight_ave = os.path.join(pathsave, f'{name}.pdf')
    col_name = 'Drug2 intensity\n weighted average'
    DPM_run_drugmis_analysis_LOD_4(average_move_num_Ben, average_move_num_NoBen, average_move_num_Ben_mis, average_move_num_NoBen_mis,
                                   col_name, filename_drug2_intensity_weight_ave, color, linestyle, name)
    # Hazard
    name = 'hz'
    filename_hz = os.path.join(pathsave, f'{name}.pdf')
    col_name = 'hz'
    DPM_run_drugmis_analysis_LOD_4(hz_Ben, hz_NoBen, hz_Ben_mis, hz_NoBen_mis, col_name, filename_hz, color, linestyle, name, format_sci=True)
    # ci of hz
    filename_hz_ci = os.path.join(pathsave, 'hz_ci.pdf')
    col_name = 'ci of hz'
    DPM_run_drugmis_analysis_LOD_4(hz_ci_Ben, hz_ci_NoBen, hz_ci_Ben_mis, hz_ci_NoBen_mis, col_name, filename_hz_ci, color, linestyle, name,
                                   format_sci=True, plot=False)
    ...
    col_name = 'hzratio'
    DPM_run_drugmis_analysis_LOD_4(hz_ratio_Ben, hz_ratio_NoBen, hz_ratio_Ben_mis, hz_ratio_NoBen_mis, col_name, filename_hzratio,
                                   color, linestyle, name)
    # ci of hz ratio
    filename_hzratio = os.path.join(pathsave, 'hzratio_ci.pdf')
    col_name = 'ci of hzratio'
    DPM_run_drugmis_analysis_LOD_4(hz_ratio_ci_Ben, hz_ratio_ci_NoBen, hz_ratio_ci_Ben_mis, hz_ratio_ci_NoBen_mis, col_name,
                                   filename_hzratio, color, linestyle, name, plot=False)
    # false rate
    name = 'falserate'
    filename_falserate = os.path.join(pathsave, f'{name}.pdf')
    col_name = 'falserate'
    DPM_run_drugmis_analysis_LOD_4(falseneg_Ben, falsepos_NoBen, falseneg_Ben_mis, falsepos_NoBen_mis, col_name, filename_falserate,
                                   color, linestyle, name, format_sci=False)
    return
...
def DPM_PeterMon_run_ci(data, n_iterations=10000, alpha=0.95):
