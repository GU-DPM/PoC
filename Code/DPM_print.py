...
''' This script defines the print functions used in the DPM (Dynamic Precision Medicine) model. '''
'''
CHECKED
'''
...
# Print an error when positional arguments are provided, as the function only accepts keyword arguments.#
def DPM_print_keywordargument_only(argv, keys, print_quotation):
    ...
    for arg_i in argv:
        if arg_i is not argv[-1]:
            ...
            DPM_print_stringandnum(arg_i, '. ', print_quotation)
    print('The function only accepts case-insensitive keyword arguments or no arguments at all.')
...
# Print error when the provided keyword arguments do not exist for this function.#
...
# Print a message when the value is not a number.#
